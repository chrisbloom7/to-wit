...
- README Usage section: added missing `doctor` subcommand, corrected `export --format` to show `md|json`, fixed `setup --hook` wording, reordered `prune` to match `towit help` output.
