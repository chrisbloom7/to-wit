...
class TestTowitSetup(unittest.TestCase):
    ...
    def test_creates_db_file_at_towit_db_path(self):
        ...
        result = run_setup(self.db_path, self.config_path)
    ...
    def test_creates_parent_directory_if_missing(self):
        ...
        nested_config = write_config(self.tmpdir, nested_path)
        self.assertFalse(os.path.exists(os.path.dirname(nested_path)))
        result = run_setup(nested_path, nested_config)
    ...
    def test_does_not_overwrite_existing_db(self):
        # First run
        run_setup(self.db_path, self.config_path)
